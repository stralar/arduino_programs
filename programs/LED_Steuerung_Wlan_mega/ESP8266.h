...
class ESP8266 {
 public:
     ...
     *
     * @param port - the port number to listen(default: 666).
     * @retval true - success.
     * @retval false - failure.
     *
     ...
     */
    bool startTCPServer(uint32_t port = 666);
     ...
     * @see uint32_t recv(uint8_t *coming_mux_id, uint8_t *buffer, uint32_t len, uint32_t timeout);
     */
    bool startServer(uint32_t port = 666);
 ...
 private:
    ...
    bool sATCIPSERVER(uint8_t mode, uint32_t port = 666);
